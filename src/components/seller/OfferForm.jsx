  ...
  return (
    <form action={action} onSubmit={handleSubmit} noValidate className="grid gap-4 rounded-md border border-white/10 bg-slate-950/50 p-4 md:grid-cols-2">
      ...
      <label className="space-y-2 text-sm text-slate-300">
        <span>Title</span>
        <input name="title" defaultValue={offer?.title ?? ""} required aria-invalid={Boolean(errors.title)} onChange={() => clearError("title")} className="w-full rounded-md border border-white/10 bg-slate-950 px-3 py-2 text-white outline-none focus:border-emerald-300" />
        <FieldError>{errors.title}</FieldError>
      </label>

      <label className="space-y-2 text-sm text-slate-300">
        <span>Status</span>
        <select name="status" defaultValue={offer?.status ?? "active"} aria-invalid={Boolean(errors.status)} onChange={() => clearError("status")} className="w-full rounded-md border border-white/10 bg-slate-950 px-3 py-2 text-white outline-none focus:border-emerald-300">
      ...
      <label className="space-y-2 text-sm text-slate-300">
        ...
        <input name="min_order_amount" type="number" min="0" step="1000" defaultValue={offer?.min_order_amount ?? 0} aria-invalid={Boolean(errors.min_order_amount)} onChange={() => clearError("min_order_amount")} className="w-full rounded-md border border-white/10 bg-slate-950 px-3 py-2 text-white outline-none focus:border-emerald-300" />
        <FieldError>{errors.min_order_amount}</FieldError>
      </label>

      <label className="space-y-2 text-sm text-slate-300">
        <span>Starts at</span>
        <input name="starts_at" type="datetime-local" defaultValue={toInputDateTime(offer?.starts_at)} aria-invalid={Boolean(errors.starts_at)} onChange={() => clearError("starts_at")} className="w-full rounded-md border border-white/10 bg-slate-950 px-3 py-2 text-white outline-none focus:border-emerald-300" />
        <FieldError>{errors.starts_at}</FieldError>
      </label>

      <label className="space-y-2 text-sm text-slate-300">
        <span>Ends at</span>
        <input name="ends_at" type="datetime-local" defaultValue={toInputDateTime(offer?.ends_at)} aria-invalid={Boolean(errors.ends_at)} onChange={() => clearError("ends_at")} className="w-full rounded-md border border-white/10 bg-slate-950 px-3 py-2 text-white outline-none focus:border-emerald-300" />
